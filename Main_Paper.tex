...
\begin{document}
	...
	\subsubsection{FATE Network Integration}
	To complement the technical and regulatory safeguards above, we incorporate a FATE (Fairness, Accountability, Transparency, and Ethics) network layer into the healthcare AI system architecture. Existing defenses such as output truncation, rate limiting, and differential privacy reduce membership inference risk at the technical level, but they do not by themselves address broader concerns related to fairness, explainability, and institutional responsibility. We therefore model FATE as a cross-cutting governance and monitoring layer that interacts with the training, deployment, inference, and audit components of the system.

	The FATE layer has four functions. \textbf{Fairness} requires continuous auditing of model outputs across demographic and clinical subgroups to detect disparate error rates, bias amplification, or privacy-utility trade-offs that disproportionately affect sensitive cohorts. \textbf{Accountability} requires model versioning, decision logging, and tamper-evident audit trails so that actions can be attributed to specific actors, workflows, or automated processes. \textbf{Transparency} requires interpretable outputs and clear documentation for clinicians and institutional stakeholders, reducing dependence on opaque confidence vectors that may simultaneously hinder clinical oversight and facilitate black-box MIA exploitation. \textbf{Ethics} requires formal oversight structures, such as review boards or deployment committees, to assess whether privacy-preserving interventions materially alter clinical utility or create inequitable outcomes.

	This layer also helps identify second-order risks introduced by MIA defenses themselves. For example, aggressive output truncation can suppress uncertainty information that clinicians may need for safe decision-making, while differential privacy may reduce accuracy more sharply for minority populations or rare-disease cohorts with limited representation. Integrating FATE into the architecture ensures that membership inference defenses are evaluated not only for privacy gain, but also for their downstream effects on fairness, clinical interpretability, and governance quality.
